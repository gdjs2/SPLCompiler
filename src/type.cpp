...
var_info_list parse_ExtDecList(tree_node *ext_dec_list_node) {
	...
	while (1) {
		...
		if (ext_dec_list_node
			...
			->children_number == 1) {
				is_array = false;
				var_name = std::string(ext_dec_list_node
						->child_first_ptr->node
						->child_first_ptr->node
						->child_first_ptr->node
						->name+4);
			...
			var_name = std::string(ext_dec_list_node
						...
						->child_first_ptr->node
						->name+4);
			length = std::stoi(ext_dec_list_node
					->child_first_ptr->node
					->child_first_ptr->node
					->child_first_ptr
					->next_child
...
var_info_list parse_DecList(tree_node *dec_list_node) {
	...
	while (1) {
		...
		->children_number == 4) {
			is_array = true;
			var_name = std::string(dec_list_node
				->child_first_ptr->node
				->child_first_ptr->node
				->child_first_ptr->node
				->child_first_ptr->node
				->name+4);
			length = std::stoi(dec_list_node
				->child_first_ptr->node
				->child_first_ptr->node
				->child_first_ptr
				->next_child
...
void _parse_Struct_Exp(tree_node *exp, struct_var_list &list) {
	...
	if (exp->children_number == 3) list.push_back(exp->child_first_ptr->next_child->next_child->node->name+4);
...
tree_node* find_first_left_value_node(tree_node* node) {
	...
	&& !strcmp(node->child_first_ptr->next_child->node->name, "DOT")) {
		...
		return node;
	} else if ((node->children_number == 4) && !strcmp(node->child_first_ptr->next_child->node->name, "LB")) {
		return node;
	} else {
		child_list_node *child = node->child_first_ptr;
		while (child) {
...
tree_node* find_first_right_value_node(tree_node *node) {
	...
	|| !memcmp(node->name, "FLOAT", 5) || !memcmp(node->name, "CHAR", 4)) {
		return node;
